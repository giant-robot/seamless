...
function Seamless(config) {
    checkForDependencies() && (this.config = $.extend(!0, {}, this.defaults, config), 
    this.cycle = 0, this.items = [], this.$container = $(this.config.containerSelector), 
    this.template = this.config.compileTemplate(this.config.template));
}

const $ = require("jquery"), Waypoint = require("waypoints/src/waypoint");
...
const checkForDependencies = function() {
    return "undefined" == typeof $ && "console" in window ? (window.console.info("Seamless needs jQuery."), 
    !1) : !("undefined" == typeof Waypoint && "console" in window) || (window.console.info("Seamless needs Waypoints."), 
    !1);
};
